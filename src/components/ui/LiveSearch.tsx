...
const LiveSearch: React.FC<LiveSearchProps> = ({ onClose, className = '', triggerVariant = 'icon', appearance = 'dark' }) => {
  ...
  return (
    <div className={className}>
      ...
      {triggerVariant === 'icon' ? (
        ...
        <button
          onClick={() => setIsOpen(true)}
          className={`w-full relative flex items-center gap-2 px-4 py-2.5 rounded-2xl transition-all duration-300 group ${appearance === 'light' ? 'bg-[#fdf9f7] border border-[#07635d]/30 text-gray-800 hover:border-[#07635d]/50' : 'bg-white/10 border border-white/20 text-white/80 hover:text-white hover:bg-white/15'}`}
        >
          <Search size={18} className={appearance === 'light' ? 'text-[#07635d]' : 'text-white/80'} />
          ...
          <div ref={searchRef} className="w-full max-w-2xl mx-4">
            ...
            <div className="relative mb-4">
              <input
                ...
                className="w-full px-6 py-4 pl-14 pr-12 text-gray-900 placeholder-gray-500 bg-white border border-gray-300 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#07635d]/30 focus:border-[#07635d]/40 transition-all duration-300 shadow-lg text-lg text-right"
                autoFocus
              />
              ...
              >
                 {isLoading ? (
                   <div className="p-8 text-center">
                     <div className="inline-block w-8 h-8 border-4 border-[#07635d] border-t-transparent rounded-full animate-spin mx-auto mb-4" />
                         ...
                         >
                           <div className="flex items-center gap-3">
                             ...
                             <div className="flex-1 min-w-0">
                                <h4 className="font-semibold text-gray-900 truncate text-right mb-1 group-hover/item:text-[#07635d] transition-colors duration-200 text-sm">
                                  {product.name}
                                </h4>
                               ...
                               <div className="flex items-center justify-end">
                                    <span className="text-xs font-semibold text-black bg-[#07635d] px-2 py-1 rounded-lg">
 ...
 <div className="w-6 h-6 rounded-lg bg-[#07635d]/10 flex items-center justify-center">
                                  <svg className="w-3 h-3 text-[#07635d]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      {searchResults.length >= 6 && (
                        <div className="p-3 border-t border-gray-100">
                          <button
                            ...
                            className="w-full py-2.5 px-4 rounded-xl font-semibold text-black transition-all duration-300 relative overflow-hidden group/btn text-sm bg-[#07635d] hover:opacity-90"
                          >
