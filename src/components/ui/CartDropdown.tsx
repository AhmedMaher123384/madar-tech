...
const CartDropdown: React.FC<CartDropdownProps> = ({ isOpen, onClose, onHoverChange }) => {
  ...
  return (
    <>
      <div 
 className="relative w-[92vw] xs:w-[95vw] sm:w-96 md:w-96 lg:w-[28rem] border border-[#07635d]/20 rounded-xl shadow-xl z-50 bg-white max-h-[75vh] overflow-hidden"
        style={{
          background: '#ffffff',
          backdropFilter: 'none',
      ...
      >
        {/* Header */}
 <div className="p-3 sm:p-4 border-b border-[#07635d]/10">
          <div className="flex items-center justify-between">
 <h3 className="text-base sm:text-lg font-bold text-[#07635d] flex items-center gap-2">
 <ShoppingCart className="w-4 h-4 sm:w-5 sm:h-5 text-[#07635d]" />
        ...
        <div className="max-h-40 xs:max-h-48 sm:max-h-64 overflow-y-auto">
          {cartItems.length === 0 ? (
            ...
            <div className="p-1 xs:p-1 sm:p-2">
              {cartItems.map((item) => (
 <div key={item.id} className="flex items-center gap-1 xs:gap-2 sm:gap-3 p-1.5 xs:p-2 sm:p-3 hover:bg-[#07635d]/5 rounded-lg xs:rounded-xl transition-colors">
                  {/* Product Image */}
 <div className="w-8 h-8 xs:w-10 xs:h-10 sm:w-12 sm:h-12 rounded-md xs:rounded-lg overflow-hidden bg-[#07635d]/10 flex-shrink-0">
                    <img
 ...
 <h4 className="text-[#07635d] text-xs sm:text-sm font-medium truncate">
                      {item.product.name}
                    </h4>
 ...
 <div className="mt-2 p-2 bg-[#07635d]/5 rounded-lg border border-[#07635d]/10">
 <div className="text-xs text-[#07635d] font-medium mb-2 flex items-center gap-1">
 <span className="w-2 h-2 bg-[#07635d] rounded-full"></span>
                          {t('product_options')}
                        </div>
 ...
 <div key={key} className="text-xs flex justify-between items-center bg-white rounded px-2 py-1 border border-[#07635d]/10">
                                <span className="text-gray-700">{key}:</span>
 <span className="text-[#07635d] font-medium">{value}</span>
                              </div>
                            ))}
 ...
 <div key={index} className="text-xs flex justify-between items-center bg-white rounded px-2 py-1 border border-[#07635d]/10">
                                <span className="text-gray-700">
                                  {option.optionName ? (i18n.language === 'ar' ? option.optionName.ar : option.optionName.en) : option.optionId}:
                                </span>
 <span className="text-[#07635d] font-medium">
 ...
 <div className="mt-2 p-2 bg-[#07635d]/10 rounded-lg border border-[#07635d]/20">
 <div className="text-xs text-[#07635d] font-medium mb-1 flex items-center gap-1">
 <span className="w-1.5 h-1.5 bg-[#07635d] rounded-full"></span>
                          {t('cart_dropdown.available_to_add')}:
                        </div>
                        <div className="space-y-1">
                          {item.product.additionalServices.slice(0, 2).map((service, index) => (
 <div key={index} className="text-xs text-gray-700 flex justify-between items-center bg-white rounded px-2 py-1 border border-[#07635d]/10">
                              <span className="flex items-center gap-1">
 <span className="text-[#07635d] text-xs">+</span>
                                {service.name}
                              </span>
 <span className="text-[#07635d] font-medium"><PriceDisplay price={service.price} /></span>
                            </div>
                          ))}
                        </div>
                        {item.product.additionalServices.length > 2 && (
 <div className="text-xs text-[#07635d]/70 mt-1 text-center">+{item.product.additionalServices.length - 2} {t('cart_dropdown.other_products')}</div>
                        )}
                      </div>
                    )}
                    
                    {/* Price Breakdown */}
 <div className="mt-3 p-3 bg-white rounded-lg border border-[#07635d]/10">
 <div className="text-xs text-[#07635d] font-medium mb-2 flex items-center gap-1">
 <span className="w-1.5 h-1.5 bg-[#07635d] rounded-full"></span>
                        {t('price_breakdown')}
                      </div>
 ...
 <span className="text-[#07635d] font-medium">
                        ...
                        {/* Divider */}
 <div className="border-t border-[#07635d]/10 my-1"></div>

                        {/* Total */}
                        <div className="flex justify-between items-center text-sm font-semibold">
 <span className="text-[#07635d]">{t('total')}</span>
 <span className="text-[#07635d]">
                            <PriceDisplay price={(() => {
 ...
 className="w-4 h-4 xs:w-5 xs:h-5 sm:w-6 sm:h-6 rounded bg-white border border-[#07635d]/20 hover:bg-[#07635d]/5 flex items-center justify-center text-[#07635d] transition-colors"
                    >
 ...
 className="w-4 h-4 xs:w-5 xs:h-5 sm:w-6 sm:h-6 rounded bg-white border border-[#07635d]/20 hover:bg-[#07635d]/5 flex items-center justify-center text-[#07635d] transition-colors"
        ...
        {/* Footer */}
        {cartItems.length > 0 && (
 <div className="p-2 xs:p-3 sm:p-4 border-t border-[#07635d]/10 bg-white">
            {/* Total */}
            <div className="flex items-center justify-between mb-2 xs:mb-3 sm:mb-4">
 <span className="text-[#07635d] font-medium text-sm sm:text-base">{t('cart_dropdown.total')}:</span>
 <span className="text-[#07635d] font-bold text-base sm:text-lg">
 ...
 className="flex-1 bg-white border border-[#07635d]/20 hover:bg-[#07635d]/5 text-[#07635d] py-2 px-2 sm:px-4 rounded-xl transition-colors text-xs sm:text-sm font-medium"
              >
 ...
 className="flex-1 bg-[#07635d] hover:bg-[#07635d] text-white py-2 px-2 sm:px-4 rounded-xl transition-all duration-300 text-xs sm:text-sm font-medium flex items-center justify-center gap-1 sm:gap-2"
              >
