...
const CartNotification: React.FC<CartNotificationProps> = ({ 
  ...
  return (
    <div className={`hidden sm:block fixed top-16 sm:top-20 ${isRTL ? 'left-2 sm:left-4' : 'right-2 sm:right-4'} z-50 max-w-xs sm:max-w-sm w-full px-2 sm:px-0`}>
      <div 
 className={`bg-white border border-[#07635d]/20 rounded-xl sm:rounded-2xl shadow-2xl p-3 sm:p-4 transform transition-all duration-500 ease-out ${
 ...
 <div className="w-6 h-6 sm:w-8 sm:h-8 bg-[#07635d] rounded-full flex items-center justify-center shadow-lg">
                <CheckCircle className="w-3 h-3 sm:w-4 sm:h-4 text-white" />
              </div>
              <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-400 rounded-full animate-ping"></div>
            </div>
 <span className="text-[#07635d] font-medium text-xs sm:text-sm">
          ...
          <button
            onClick={handleClose}
 className="text-gray-500 hover:text-[#07635d] transition-colors p-1 hover:bg-[#07635d]/10 rounded-full"
          >
 ...
 <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-lg sm:rounded-xl overflow-hidden bg-white flex-shrink-0 border border-[#07635d]/20 shadow-lg">
            <img
 ...
 <h4 className="text-[#07635d] text-xs sm:text-sm font-bold truncate mb-1 leading-tight">
              {product.name}
            </h4>
            {/* Price Breakdown */}
 <div className="mb-2 p-2 bg-white rounded-lg border border-[#07635d]/10">
              <div className="space-y-1">
                {/* Base Price */}
                <div className="flex justify-between items-center text-xs">
                  <span className="text-gray-700">{t('base_price')}:</span>
 <PriceDisplay price={product.price * quantity} className="text-[#07635d] font-medium" />
                </div>
 ...
 className="text-[#07635d] font-medium"
                    />
                  </div>
                )}
 ...
 className="text-[#07635d] font-medium"
                ...
                {/* Total Price */}
 <div className="flex justify-between items-center text-sm border-t border-[#07635d]/10 pt-1">
 <span className="text-[#07635d] font-bold">{t('total')}:</span>
                  <PriceDisplay 
                    price={(() => {
 ...
 className="text-[#07635d] font-bold"
                  />
                </div>
              </div>
            </div>
 ...
 <span className="text-white font-medium bg-[#07635d] px-1.5 sm:px-2 py-1 rounded-full text-xs shadow-sm">
                {quantity}
              </span>
            </div>
 ...
 <span key={index} className="text-xs bg-white px-2 py-1 rounded-full text-gray-700 truncate border border-[#07635d]/10">
 ...
 <span key={key} className="text-xs bg-white px-2 py-1 rounded-full text-gray-700 truncate border border-[#07635d]/10">
                        {key}: {typeof value === 'object' ? JSON.stringify(value) : String(value)}
                      </span>
                    ))
                  )}
                  {optionsCount > 1 && (
 <span className="text-xs text-gray-600 bg-white px-2 py-1 rounded-full border border-[#07635d]/10">
 ...
 <span key={index} className="truncate bg-white px-2 py-1 rounded-full text-xs border border-[#07635d]/10 text-gray-700">
                    {getLocalizedAddOnContent('name', addon)} (+{addon.price})
                  </span>
                ))}
                {addOnsCount > 1 && (
 <span className="text-gray-500 bg-white px-2 py-1 rounded-full text-xs ml-1 border border-[#07635d]/10">
                    +{addOnsCount - 1}
                  </span>
                )}
              </div>
 ...
 <div className="bg-white rounded-lg sm:rounded-xl p-2 sm:p-3 mb-3 sm:mb-4 border border-[#07635d]/10">
          <div className="flex items-center justify-between text-xs sm:text-sm">
            <span className="text-gray-700 flex items-center gap-1">
              <ShoppingCart className="w-3 h-3" />
              {t('cart_notification.total_products_in_cart')}:
            </span>
 <span className="text-[#07635d] font-bold bg-white px-2 py-1 rounded-full border border-[#07635d]/10">
 ...
 className="flex-1 bg-white hover:bg-[#07635d]/5 text-[#07635d] py-2 sm:py-2.5 px-2 sm:px-3 rounded-lg sm:rounded-xl transition-all duration-300 text-xs sm:text-sm font-medium flex items-center justify-center gap-1 sm:gap-2 border border-[#07635d]/20 hover:border-[#07635d]/30 hover:shadow-lg group"
          >
 ...
 className="flex-1 bg-[#07635d] hover:bg-[#07635d] text-white py-2 sm:py-2.5 px-2 sm:px-3 rounded-lg sm:rounded-xl transition-all duration-300 text-xs sm:text-sm font-medium flex items-center justify-center gap-1 sm:gap-2 shadow-lg hover:shadow-xl transform hover:scale-105 group"
          >
